<template>
  <v-menu content-class="no-border-radius" open-on-hover offset-y nudge-width="1" v-if="auth" key="user-menu">
  ...
  <v-menu v-else-if="!auth" offset-y :close-on-content-click="auth" key="loginAndRegister-menu">
    <template v-slot:activator="{on}">
      <v-btn v-on="on" class="mt-auto mb-auto ml-auto mr-4" color="#29B6F6" elevation="0" dark>Вход</v-btn>
    </template>
    <v-form ref="form" v-model="valid" lazy-validation>
      <v-card>
        ...
        <v-list v-else-if="reg">
          ...
          <v-list-item>
            <v-text-field v-model="passwordReg" :rules="passwordRegRules" placeholder="Повторите пароль"></v-text-field>
          </v-list-item>
        </v-list>
        ...
        <v-card-actions v-else-if="reg">
          <v-btn :disabled="!valid" @click="registerNewAccount" color="secondary" block>Создать аккаунт</v-btn>
        </v-card-actions>
      </v-card>
    </v-form>
  </v-menu>
